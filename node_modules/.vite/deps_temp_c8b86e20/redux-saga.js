...
function channel2(buffer2) {
  ...
  var takers = [];
  {
    check(buffer2, buffer, INVALID_BUFFER);
  }
...
function runChannelEffect(env, _ref12, cb) {
  var pattern = _ref12.pattern, buffer2 = _ref12.buffer;
  var chan = channel2(buffer2);
...
function proc(env, iterator2, parentContext, parentEffectId, meta, isRoot, cont) {
  if (iterator2[asyncIteratorSymbol]) {
  ...
  function next(arg, isErr) {
    try {
      ...
      if (isErr) {
        ...
        mainTask.status = CANCELLED2;
        next.cancel();
        result = func(iterator2.return) ? iterator2.return(TASK_CANCEL) : {
          done: true,
          value: TASK_CANCEL
        };
      } else if (shouldTerminate(arg)) {
        result = func(iterator2.return) ? iterator2.return() : {
          done: true
        };
      } else {
...
function runSaga(_ref, saga) {
  ...
  var iterator2 = saga.apply(void 0, args);
  {
    check(iterator2, iterator, NON_GENERATOR_ERR);
  }
  var effectId = nextSagaId();
  if (sagaMonitor) {
...
function sagaMiddlewareFactory(_temp) {
  var _ref = _temp === void 0 ? {} : _temp, _ref$context = _ref.context, context = _ref$context === void 0 ? {} : _ref$context, _ref$channel = _ref.channel, channel3 = _ref$channel === void 0 ? stdChannel() : _ref$channel, sagaMonitor = _ref.sagaMonitor, options = _objectWithoutPropertiesLoose(_ref, _excluded);
  var boundRunSaga;
  {
...
export {
  ...
  buffers,
  channel2 as channel,
  sagaMiddlewareFactory as default,
  detach,
  eventChannel,
  isEnd,
  multicastChannel,
